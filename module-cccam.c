...
int32_t cc_parse_msg(struct s_client *cl, uint8_t *buf, int32_t l) {
	...
	char msgname[20];
	cs_debug_mask(cl->typ=='c'?D_CLIENT:D_READER, "%s parse_msg=%s", getprefix(), cc_get_msgname(buf[1],msgname,sizeof(msgname)));
			...
			if (retry) {
				...
				for (i = 0; i < cfg.max_pending; i++) {
					if (cl->ecmtask[i].idx == ecm_idx) {
						...
						cl->pending--;
						er->rc = E_NOTFOUND;
						uint8_t rcEx=(buf[1]==MSG_CW_NOK1)?E2_CCCAM_NOK1:E2_CCCAM_NOK2;
						write_ecm_answer(rdr, er, E_NOTFOUND, rcEx, NULL, NULL);
						break;
					}
				}
			}
